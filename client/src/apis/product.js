...
export const listRecommendProducts = (filter) => {
    const {
        search,
        sortBy,
        order,
    ...
    return fetch(
        `${API}/active/products?search=${search}&rating=${rating}&minPrice=${minPrice}&maxPrice=${maxPrice}&productId=${productId}&sortBy=${sortBy}&order=${order}&limit=${limit}&page=${page}`,
        {
            method: 'GET',
            headers: {
                Accept: 'application/json',
...
export const deleteProduct = (userId, token, productId, storeId) => {
    return fetch(`${API}/product/delete/${productId}/${storeId}/${userId}`, {
        method: 'DELETE',
        headers: {
            Accept: 'application/json',
            Authorization: `Bearer ${token}`,
